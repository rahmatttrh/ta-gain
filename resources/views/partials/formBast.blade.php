<x-app-layout>
 ...
 @endsection
 <div class="flex">
   <a href="{{route('finish.kordinator')}}" class=" mb-2 w-12 f item-center  mr-2 shadow-xs dark:bg-gray-700 text-blue-400 font-bold  p-3 rounded">
   ...
   <x-header><span class="font-bold">FORM</span> BAST {{$order->site}}</x-header>
 </div>
   
 <a href="{{route('download.bast.master', $order)}}"  class="mb-6 text-xs text-center py-4 mr-1 bg-green-500 text-gray-100 py-1 px-4  rounded-xl leading-tight hover:bg-green-600">
  <span>Download Master BAST</span>
 </a>
...
<div class="w-full mb-5 py-4 px-4 dark:bg-gray-900  overflow-hidden rounded-lg shadow-xs">
     ...
     <div class="container mx-auto">
         ...
         <form action="{{route('upload.bast', $order)}}" method="POST" enctype="multipart/form-data" >
             @csrf
